...
namespace ViewSalesPromProducts
{
    public partial class frmDel : Form
    {
        ...
        {
            dtTovar = Config.connectMain.getGoodToCatalog(ean.Trim(), false);
            ...
            if ((int)dtTovar.Rows[0]["id"] == -1)
            {
                ...
                if (dtTovar.Columns.Contains("id_tovar"))
                {
                    int id_tovar = (int)dtTovar.Rows[0]["id_tovar"];
                    DataTable dtResult = Config.connectMain.setCatalogPromotionalTovars(id_tovar, 0.0M, 0.0M, true, false);                    
                }
                else
